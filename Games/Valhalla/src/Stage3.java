...
public class Stage3 extends Scenario {
    private final int waveCount = 6; // Número de ondas no cenário
    private double wavelength = 25; // Largura da onda
    private double amplitude = 15; // Amplitude da onda
    ...
    protected void initializeEnemies(Enemy.Difficulty difficulty) {
        ...
        for (int i = 0; i < 10; i++) {
            Enemy enemy = new Enemy("Inimigo" + (i + 1), difficulty, Color.RED, 800, 120 + (i * 30));
            enemies.add(enemy);
            root.getChildren().add(enemy.getShape());
        }
    }
